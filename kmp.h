...
int kmp_size(kmp_state state) {
    int result = sizeof(char*) + sizeof(char) * (state.period_len + 1) + sizeof(int) * 5 + sizeof(int*);
    int limit = (state.period_len == state.m) ? state.period_len : (state.period_len << 1);
    result += limit * sizeof(int);
    return result;
}
...
int get_failure_i(kmp_state state, int i) {
    ...
    if (i < double_period) return state.failure[i];
    if ((i == state.m - 1) && (state.has_break)) return state.break_failure;
    return state.failure[double_period - 1] + i - double_period + 1;
}
...
kmp_state kmp_build(char* P, int m, int p_len) {
    ...
    if (((state.failure[m - 1] + 1) << 1) >= m) {
        state.period_len = m - state.failure[m - 1];
        state.P = realloc(state.P, state.period_len * sizeof(char));
        state.failure = realloc(state.failure, (state.period_len << 1) * sizeof(int));
